...
charIndex::Insert(const char *match_data, HostBranch *toInsert)
{
  ...
  while (true) {
    ...
    if (index == 255) {
      ...
      ink_hash_table_insert(illegalKey, match_start, toInsert);
      break;
    }
  ...
  while (true) {
    ...
    if (index == 255) {
      if (illegalKey == nullptr) {
        ...
        if (ink_hash_table_lookup(illegalKey, match_start, &hash_lookup)) {
...
HostLookup::InsertBranch(HostBranch *insert_in, const char *level_data)
{
  ...
  case HOST_TERMINAL:
    ...
    break;
  case HOST_HASH:
    ink_hash_table_insert(static_cast<InkHashTable *>(insert_in->next_level), level_data, new_branch);
    break;
  case HOST_INDEX:
  ...
  case HOST_ARRAY:
    if (static_cast<hostArray *>(insert_in->next_level)->Insert(level_data, new_branch) == false) {
      ...
      ink_hash_table_insert(new_ht, level_data, new_branch);
...
HostLookup::FindNextLevel(HostBranch *from, const char *level_data, bool bNotProcess)
{
  ...
  case HOST_HASH:
    ...
    if (ink_hash_table_lookup(hash, level_data, &lookup)) {
