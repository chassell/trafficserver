...
public:
  ...
  {
    thread_group.emplace_back(ats_strdup(name));

    ink_release_assert(thread_group.size() <= MAX_EVENT_TYPES); // check for overflow

    n_thread_groups = thread_group.size();
  ...
  */
  ...
  void schedule_spawn(EThread::InitFxn f, EventType ev_type)
  {
