...
template <class T_CONTOBJ> struct TSContObj {
public:
  TSContObj(TSCont cb) : _heldCb(cb) {}

  TSContObj(TSEventFunc fxn, T_CONTOBJ *obj) : _heldCb(TSContCreate(fxn, TSMutexCreate())) { TSContDataSet(_heldCb, obj); }

  ~TSContObj() { TSContDestroy(_heldCb); }

  T_CONTOBJ &operator*() { return *static_cast<T_CONTOBJ *>(TSContDataGet(_heldCb)); }
  T_CONTOBJ *operator->() { return static_cast<T_CONTOBJ *>(TSContDataGet(_heldCb)); }
...
{
public:
  PluginState(int argc, const char *argv[]) { Async::execute<AsyncTimer>(this, &_cfgCheck, std::shared_ptr<Mutex>()); }
  ...
  static int handleScanEvent(TSCont cont, TSEvent event, void *arg);

  void handleAsyncComplete(AsyncTimer &timer) override // config timer
  {
    configChanged();
  }
...
public:
  ...
  void handleAsyncComplete(PluginState &timer) override; // cancel of scan
};

void
PluginState::configChanged()
{
  auto cb = TSCacheScan(cb, nullptr, 512000);
}
