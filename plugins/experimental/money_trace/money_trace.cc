...
MT::moneyTraceHdr(const char *mt_request_hdr)
{
  ...
  strcpy(new_mt_hdr, temp_str.str().c_str());
  length = strlen(new_mt_hdr);

  return TSstrndup(new_mt_hdr, length);
}
